...
mod tests {
    ...
    #[tokio::test]
    async fn standby_falls_back_to_snapshot_when_event_pull_fails() {
        let target_dir = tempfile::tempdir().unwrap();
        let snapshot = HaStateSnapshot {
            last_seq: 12,
            checksum: String::new(),
            contacts: ContactStateSnapshot {
                contacts: vec![ContactBinding {
                    aor: "sip:400@example.com".to_string(),
                    contact: "sip:400@127.0.0.1:5064".to_string(),
                    source: "127.0.0.1:50000".to_string(),
                    expires_at_epoch_ms: expires_at(Duration::from_secs(60)),
                }],
            },
        ...
        for _ in 0..50 {
            if state.lookup("sip:400@example.com").await.is_some()
                && target_persistence.last_applied_seq().await.unwrap() == 12
            {
                break;
            }
            tokio::time::sleep(Duration::from_millis(20)).await;
        }
        assert!(state.lookup("sip:400@example.com").await.is_some());
        assert_eq!(target_persistence.last_applied_seq().await.unwrap(), 12);
        let _ = shutdown_tx.send(true);
        pull_task.await.unwrap().unwrap();
        peer_task.abort();
    }
